...
void simple_boot(int fd, uint32_t boot_addr, const uint8_t *buf, unsigned n) { 
    ...
    if (op == BOOT_ERROR) {
      output("code collision\n");
      return;
    }
    if (op != GET_CODE) {
      output("expected GET_CODE from pi, received %d instead\n", op);
      return;
    }
    uint32_t recv_crc = trace_get32(fd);
    if (recv_crc != crc) {
      output("expected crc = %x, got crc = %x instead\n", crc, recv_crc);
      return;
    }
    ...
    op = get_op(fd);
    if (op == BOOT_ERROR) {
      output("pi crc check failed\n");
      return;
    }
    if (op != BOOT_SUCCESS) {
      output("expected BOOT_SUCCESS from pi, received %d instead\n", op);
      return;
    }
