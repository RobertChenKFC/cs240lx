...
static inline void gpio_set_on_raw(unsigned pin) {
  /*
  unsigned addr = gpio_set0 + 4 * (pin / 32);
...
static inline void gpio_set_off_raw(unsigned pin) {
  /*
  unsigned addr = gpio_clr0 + 4 * (pin / 32);
	unsigned offset = pin % 32;
	PUT32(addr, 1 << offset);
  */
...
static void pix_sendbyte(unsigned pin, uint8_t b) {
  /*
  pix_sendbit(pin, (b >> 7) & 1);
  pix_sendbit(pin, (b >> 6) & 1);
  pix_sendbit(pin, (b >> 5) & 1);
  pix_sendbit(pin, (b >> 4) & 1);
  pix_sendbit(pin, (b >> 3) & 1);
  pix_sendbit(pin, (b >> 2) & 1);
